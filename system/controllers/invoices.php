...
switch ($action) {
    ...
    case 'multi_view':
        ...
        $app->emit('invoices_multi_view', [&$invoice_ids]);
        $i = 0;
        $multi_ds = array();
        $total_invoice_ids = '';
        foreach ($invoice_ids as $id) {

            // $d = Invoice::find($id)->toArray(); // Save Data according to Invoice ID;
            $d = ORM::for_table('sys_invoices')->find_one($id);
            if ($d) {
                ...
                $items = ORM::for_table('sys_invoiceitems')->where('invoiceid', $id)->order_by_asc('id')->find_many();
                // $ui->assign('items', $items); // Should Register items to tempData

                // find related transactions
                // $trs_c = Transaction::where('iid', $id)->count();
                // $trs = Transaction::where('iid', $id)->order_by_desc('id')->get()->toArray();
                $trs_c = ORM::for_table('sys_transactions')->where('iid', $id)->count();
                $trs = ORM::for_table('sys_transactions')->where('iid', $id)->order_by_desc('id')->find_many();
                // $ui->assign('trs', $trs);    Should Register trs to tempData
                // $ui->assign('trs_c', $trs_c);    Should Register trs_c to tempData
                $emls_c = ORM::for_table('sys_email_logs')->where('iid', $id)->count();
                $emls = ORM::for_table('sys_email_logs')->where('iid', $id)->order_by_desc('id')->find_many();
                // $ui->assign('emls', $emls);  Should Register emails to tempData
                // $ui->assign('emls_c', $emls_c); Should Regsiter Emails to tempData
                // $a = Contact::all()->keyBy($d['userid']);
                $a = ORM::for_table('crm_accounts')->find_one($d['userid']);
                // $ui->assign('a', $a); Should Regsiter a to tempData
                // $ui->assign('d', $d); Should Register d to tempData
                ...
                $i_due = number_format($i_due, 2, $config['dec_point'], $config['thousands_sep']);
                // $ui->assign('i_due', $i_due); Should Register i_due to TempData;
                $cf = ORM::for_table('crm_customfields')->where('showinvoice', 'Yes')->order_by_asc('id')->find_many();
                // $ui->assign('cf', $cf); Shoud Register cf to tempData
                $upload_max_size = ini_get('upload_max_filesize');
                $post_max_size = ini_get('post_max_size');
                // $ui->assign('upload_max_size', $upload_max_size);    Should Register upload_max_size to tempData
                // $ui->assign('post_max_size', $post_max_size);    Should Register post_max_size to tempData

                //count attatchments
                $inv_files = Invoice::files($id);
                $inv_files_c = count($inv_files);
                // $ui->assign('inv_files_c', $inv_files_c); Should Register invoice_attached files to tempData
                // $ui->assign('inv_files', $inv_files);    Should Register invoice_attached files to tempData
                $access_logs = ORM::for_table('ib_invoice_access_log')->where('iid', $id)->order_by_desc('id')->find_array();
                // $ui->assign('access_logs', $access_logs);    Should Register access logs

                $total_invoice_ids .= '&ids[]=';
                $total_invoice_ids .= $d['id'];
                ...
                $temp_data = (object) array(
                    ...
                    'inv_files_c' => $inv_files_c,
                    'inv_files' => $inv_files,
                    'access_logs' => $access_logs
                );
            }
            $multi_ds[$i] = $temp_data;
            $i++;
        }

        $ui->assign("multi_datas", $multi_ds);
        ...
        $x_html = '';
        Event::trigger('view_multi_invoice');
        $ui->assign('x_html', $x_html);
        ...
        view('invoices_multi_view', [
            'total_invoice_ids' => $total_invoice_ids,
            'currencies' => Currency::all(),
            'currency_rate' => $currency_rate,
            'company' => $company,
            'quote' => $quote,
    ...
    case 'list':
        ...
        $filter_paid_status = _post('unpaid_checkbox');
        $filter_from_date = _post('fromDateFilter');
        $filter_to_date = _post('toDateFilter');
        ...
        // var_dump($filter_to_date);
        // var_dump($filter_paid_status);
        
        if ($filter_from_date == ""){
            $filter_from_date = "1990-01-01";
        }

        if($filter_to_date == ""){
            $filter_to_date = "2050-01-01";
        }
        ...
        foreach($d as $ds){
            ...
            if($filter_from_date < $ds['date'] && $ds['date'] < $filter_to_date){
                if($filter_paid_status == "checked"){
                    if($ds['status'] == 'Unpaid' || $ds['status'] == 'Partially Paid'){
                        $filter_datas[$i] = $ds;
                    }
                } else {
                    $filter_datas[$i] = $ds;
        ...
        $ui->assign('d', $filter_datas);
        $ui->assign('filter_unpaid_checked', $filter_paid_status);
        $ui->assign('filter_from_date',$filter_from_date);
        $ui->assign('filter_to_date', $filter_to_date);
